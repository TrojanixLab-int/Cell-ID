...
class AdvancedCellAnalyzer:
    def __init__(self, root):
        ...
        self.root.title("Mobilfunkzellen-Analyzer v2.1.1 - M. Trojan")
    ...
    def save_report_to_file(self):
        ...
        for i in range(1, 100):
            ...
            if not os.path.exists(fp):
                break
        t = "   "
        sim_data = self.lbl_sim.cget("text").split("\n")
...
(C) 2026 - Cell-ID: Mobilfunkzellen-Analyzer Version 2.1.1 - Entwickelt von  M. Trojan, Trojanix Lab int.
...
Raw/Diag. & Terminal-Funktionen

- Raw/Diag.: Öffnet das Analyse-Terminal für den direkten Datenaustausch mit der Modem-Hardware.
...
- Alles kopieren: Schreibt den Inhalt des Terminal-Fensters in die Zwischenablage.
- Alles Speichern: Exportiert den Debug-Log als Textdatei (RawDebug_...) in den Ordner "gespeichert" und öffnet die Datei sofort zur Ansicht.
...
- Mobilfunk-Vorgaben: Vom Netz übermittelte Konfigurationsparameter für das Gerät:
  - RACH: Random Access Channel (Zugriffssteuerung).
  - PUSCH: Physical Uplink Shared Channel (Datenkanal-Parameter).
...
03	Telefónica DE	
04	Vodafone DE	
05	O2 Telefónica DE (ehemals E-Plus)
06	Telekom DE	
07	Telefónica DE	
08	O2 Telefónica DE (ehemals O2)
09	Vodafone DE	
10	DBInfraGo AG	
11	Telefónica DE	
13	BAAINBw	Bundeswehr
14	Lebara Limited	
15	Airdata
22	sipgate Wireless
23	1&1 Mobilfunk	
43	Lycamobile	
60	DBInfraGo AG	
72	Ericsson (Testnetz/Hersteller)
73	Nokia (Testnetz/Hersteller)
74	Ericsson (Testnetz/Hersteller)
78	T-Mobile
98	nicht-öffentlich (Privatnetze/Campusnetze)
...
Die Nutzung der Software erfolgt ausdrücklich auf eigene Gefahr. Der Entwickler übernimmt keinerlei Haftung für direkte oder indirekte Schäden, die durch eine unsachgemäße Behandlung des Programms oder der verwendeten Hardware (PC, Modem, Mobilfunkendgeräte) entstehen. Dies gilt insbesondere für Fehlfunktionen oder Hardwaredefekte, die durch die manuelle Eingabe von Steuerbefehlen über das Diagnose-Terminal hervorgerufen werden könnten. Ein Anspruch auf Schadersatz bei Datenverlust oder Folgeschäden am Betriebssystem oder der Hardware ist ausgeschlossen."""
    ...
    def handle_incoming_call(self, number="Unbekannt"):
        ...
        self.call_window.title("☎ ANRUF")
        import os
        icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Cell-ID.ico")
        if os.path.exists(icon_path):
            new_win.iconbitmap(icon_path)
    ...
    def finish_call(self):
        ...
        log_text = f"[{time_str}] ☎ Anruf von {self.call_number} (CID: {cid}) → {status}\n"
        ...
        while self.running:
            if self.ser and self.ser.is_open:
                try:
                    ...
                    clip_match = re.search(r'\+CLIP:\s*"([^"]+)"', res_csq)
                    if clip_match:
                        self.handle_incoming_call(number=clip_match.group(1))
                    ...
                    if "RING" in line:
                        ...
                        if match:
                            self.handle_incoming_call(number=match.group(1))
    ...
    def show_debug_window(self):
        ...
        tk.Button(btn_frame, text="Alles speichern", command=lambda: self.save_raw_debug_to_file(self.debug_text_widget), bg="#27aeff", fg="white", width=15).pack(side=tk.LEFT, padx=5)
